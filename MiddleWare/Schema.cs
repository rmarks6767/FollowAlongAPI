...
//TODO 1.7.b rename the reference to the GraphQL Schema type (because it has the same name as our class)
...
{
    //TODO 1.7.a First, add inheritance to the GraphQL.Types.Schema, we can do it by adding a reference to the schema under a different name using GQLSchema = GraphQL.Types.Schema to the namespace
    public class Schema : GQLSchema
    {
        //TODO 1.8.a Now make a constructor for our new Schema type
        public Schema(IServiceProvider serviceProvider) : base(serviceProvider) //TODO 1.8.b To resolve the types, GraphQL uses IServiceProvider (This comes from all the types that are injected in the Startup class)
        {
...
//TODO 1.9 The Schema is now all done so let's head over to the RootQuery class and set that one up.